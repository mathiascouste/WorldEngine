...
public class WorldGenerator {
	...
	private void generateEntity(Random rand) {
		this.generateFauna(rand);
	}

	private void generateFauna(Random rand) {
		int MAX_FAUNA = 100;
		...
		for (Area a : this.world.getAreas()) {
			...
			double probaTree = ((double) MAX_FAUNA) / ((double) cptOkGround);
			for (int x = posX; x < Area.SIZE + posX; x++) {
				for (int y = posY; y < Area.SIZE + posY; y++) {
					int ground = a.getGround(x, y);
					if (ground == Ground.EARTH || ground == Ground.GRASS) {
						if (rand.nextFloat() <= probaTree) {
							Tree tree = new Tree(a, x+rand.nextDouble(), y+rand.nextDouble(), 0);
							a.addEntity(tree);
						}
					}
				}
